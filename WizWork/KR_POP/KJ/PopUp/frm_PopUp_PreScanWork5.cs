...
namespace WizWork
{
    public partial class frm_PopUp_PreScanWork5 : Form
    {
        ...
        // 각 품명의 라벨 리스트
        public Dictionary<string, List<ChildLabel>> dicSubList = new Dictionary<string, List<ChildLabel>>();

        // 한민 테크용. 투입한 라벨 각각 1:1 로 실적으로 분리시켜서 넣기 위한 변수
        public Dictionary<string, double> dicSubLabels = new Dictionary<string, double>();

        public frm_PopUp_PreScanWork5()
        {
            InitializeComponent();
            SetScreen();  //TLP 사이즈 조정
        }
        public frm_PopUp_PreScanWork5(string strProcessID, string strMachineID, string strMoldID)
        {
            InitializeComponent();
        ...
        public frm_PopUp_PreScanWork5(string strProcessID, string strMachineID, string strMoldID, List<ChildLabel> lstChildLabel)
        {
            InitializeComponent();
            ...
            try
            {
                foreach(string Key in dicSubList.Keys)
                {
                    ...
                    if (lstCL.Count > 1)
                    {
                        ...
                        // 선스캔 먼저 등록
                        dicSubLabels.Add(ALot.LabelID, ProdCapaA);
                        ...
                        for (int i = 1; i < lstCL.Count; i++)
                        {
                            ...
                            // 후 라벨들 등록
                            dicSubLabels.Add(BLot.LabelID, MoveQty);
                            ...
                            //tOUTWARE.sUnitClss = Frm_tprc_Main.g_tBase.OrderUnit;
                            ...
                            //tSTUFFIN.UnitClss = Frm_tprc_Main.g_tBase.OrderUnit;
                            //tSTUFFIN.TotRoll = "1";
                        ...
                        // 한민 테크용 2020.08.11
                        // 일단 하위품이 하나밖에 없다는 전제 하에 
                        // 첫번째 품명 라벨들 등록 후 true 반환
                        return true;
                    }
                }
